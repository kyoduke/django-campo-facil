...
class Review(models.Model):
    ...
    football_field = models.ForeignKey(
        FootballField, on_delete=models.CASCADE, related_name="reviews"
    )
    ...
    def clean(self):
        ...
        if (
            Review.objects.filter(
                football_field=self.football_field, author=self.author, is_active=True
            )
            .exclude(pk=self.pk)
            .exists()
            > 0
            ...
            raise ValidationError(
                _("You have already submitted a review for this football field.")
            )
