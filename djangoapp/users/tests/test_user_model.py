...
class TestUserModel:
    ...
    def test_if_superuser_has_superuser_attributes(self):
        superuser = User.objects.create_superuser(email='test@test.com', password='test@1234')
        assert superuser.is_staff is True
        assert superuser.is_superuser is True

    def test_get_short_name_method(self):
        user_1 = User.objects.create_user(first_name='Testing', email='test@test.com', password='abc123')
        user_2 = User.objects.create_user(email='shortnametest@test.com', password='abc123')
